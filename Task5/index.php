...
$where = [];
$params = [];
if($filter === 'mine'){ $where[]="username=?"; $params[]=$username; }
if($search){ $where[]="(title LIKE ? OR content LIKE ?)"; $params[]="%$search%"; $params[]="%$search%"; }
...
<head>
    ...
    <style>
        ...
        .like-btn { cursor: pointer; font-size: 1.2em; margin-left: 8px; text-decoration: none; }
...
<div class="container">
    ...
    <div class="posts">
        ...
        if($posts){
            foreach($posts as $p){
                $liked = in_array($p['id'], $_SESSION['liked_posts']);
                echo "<div class='post-card' id='post-".$p['id']."'>";
                ...
                echo "<span>Likes: <span class='like-count'>".$p['likes']."</span></span>";
                echo "<a href='#' class='like-btn ".($liked?'liked':'')."' data-id='".$p['id']."'>❤️</a>";
                echo " | Posted on ".$p['created_at']." by <strong>".htmlspecialchars($p['username'])."</strong>";
                echo "</div>";
                echo "</div>";
            }
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.like-btn').forEach(btn=>{
        btn.addEventListener('click', function(e){
            ...
            let likeCountEl = this.parentElement.querySelector('.like-count');
            let btnEl = this;
            ...
            .then(data=>{
                if(data.success){
                    ...
                    // Scroll to this post to keep cursor
